...
function NebulaSphere() {
  ...
  useFrame((state) => {
    ...
    const currentScene = useStore.getState().currentScene;

    // ─ Update shader uniforms ─
    meshRef.current.material.uniforms.uTime.value = time || state.clock.elapsedTime;
    meshRef.current.material.uniforms.uScroll.value = scroll || 0;

    // ─ Set static premium colors (Base: deep black, Primary: warm amber, Secondary: electric blue) ─
    ...
    switch (currentScene?.id) {
      case SCENES.HERO.id:
        intensity = 0.8;
        break;
      case SCENES.ABOUT.id:
        intensity = 0.6;
        break;
      case SCENES.EXPERIENCE.id:
        intensity = 0.6;
        break;
      case SCENES.PROJECTS.id:
        intensity = 0.7;
        break;
      case SCENES.SKILLS.id:
        intensity = 0.5;
        break;
      case SCENES.CONTACT.id:
        intensity = 0.3;
        break;
      default:
        intensity = 1.0;
    ...
    meshRef.current.rotation.y = (time || state.clock.elapsedTime) * 0.008;
    meshRef.current.rotation.z = (time || state.clock.elapsedTime) * 0.003;
  });
...
function LightingController() {
  ...
  useFrame((state) => {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Section-Reactive Intensity (with static Warm Amber lightColor)
    // ─────────────────────────────────────────────────────────────────────
    let baseDirIntensity = 0.25;
    let baseAmbientIntensity = 0.12;
    let pointIntensity = 0.3;
    let lightColor = 0xb58038;

    if (currentScene?.id === SCENES.HERO.id) {
      baseDirIntensity = 0.7;
      baseAmbientIntensity = 0.15;
      pointIntensity = 0.4;
    } else if (currentScene?.id === SCENES.ABOUT.id) {
      baseDirIntensity = 0.4;
      baseAmbientIntensity = 0.12;
      pointIntensity = 0.2;
    } else if (currentScene?.id === SCENES.EXPERIENCE.id) {
      baseDirIntensity = 0.5;
      baseAmbientIntensity = 0.12;
      pointIntensity = 0.3;
    } else if (currentScene?.id === SCENES.PROJECTS.id) {
      baseDirIntensity = 0.6;
      baseAmbientIntensity = 0.14;
      pointIntensity = 0.4;
    } else if (currentScene?.id === SCENES.SKILLS.id) {
      baseDirIntensity = 0.4;
      baseAmbientIntensity = 0.12;
      pointIntensity = 0.2;
    } else if (currentScene?.id === SCENES.CONTACT.id) {
      baseDirIntensity = 0.3;
      baseAmbientIntensity = 0.1;
      pointIntensity = 0.1;
    }

    // ─ Directional light: animated intensity + position ─
    const dirIntensity = (baseDirIntensity + Math.sin(scroll * Math.PI * 3) * 0.3) *
                          (0.8 + Math.sin(time * 0.3) * 0.2);
    
    dirLightRef.current.intensity = dirIntensity;
    dirLightRef.current.position.x = 15 * Math.sin(scroll * Math.PI * 2 + time * 0.1);
    dirLightRef.current.position.y = 10 + 8 * Math.cos(scroll * Math.PI * 2);
    dirLightRef.current.position.z = 5 + 10 * Math.cos(scroll * Math.PI * 2 + time * 0.05);
    dirLightRef.current.color.setHex(lightColor);

    // ─ Ambient light: provides fill ─
    if (ambientLightRef.current) {
      ambientLightRef.current.intensity = baseAmbientIntensity;
      ambientLightRef.current.color.setHex(0xe0e6ed);
    }

    // ─ Point light: adds depth and drama ─
